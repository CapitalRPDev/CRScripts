if Config.StaffPerms['add'].ENABLE then
    TriggerEvent('chat:addSuggestion', '/'..Config.StaffPerms['add'].chat_command, Locale('chatsuggestion_vehicle_add'), {{ name=Locale('chatsuggestion_playerid_1'), help=Locale('chatsuggestion_playerid_2')}})
...
if Config.StaffPerms['delete'].ENABLE then
    TriggerEvent('chat:addSuggestion', '/'..Config.StaffPerms['delete'].chat_command, Locale('chatsuggestion_vehicle_delete'))
...
if Config.StaffPerms['plate'].ENABLE then
    TriggerEvent('chat:addSuggestion', '/'..Config.StaffPerms['plate'].chat_command, Locale('chatsuggestion_vehicle_plate'))
...
if Config.StaffPerms['keys'].ENABLE then
    TriggerEvent('chat:addSuggestion', '/'..Config.StaffPerms['keys'].chat_command, Locale('chatsuggestion_vehicle_keys'))
