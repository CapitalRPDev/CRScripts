if Config.InsideGarage.ENABLE then
                                        ...
                                        if not Config.Impound or GarageInfo[cd].impound == 0 then
                                            if CheckProperty_inside(cd) then
                                                if not Config.UniqueGarages or CurrentGarage == GarageInfo[cd].garage_id or InProperty == true then
                                                    ...
                                                    InsideAction(Locale('vehicle_is_in')..' <b>'..Locale('garage')..' '..GarageInfo[cd].garage_id..'</b>')
                                                end
                                            end
                                        else
                                            InsideAction(Locale('vehicle_is_in_the')..' <b>'..GetImpoundName(GarageInfo[cd].impound)..'</b>')
                                        end
                                    end
                                end
                            end
            ...
            while true do
                ...
                if InGarage and MyCars ~= nil then
                    ...
                    if dist2 > 100 then
                        ...
                        DrawTextUI('hide')
                        DeleteGarageTargetZone('garage_exit')
                    end
                else
                    break
                end
    ...
    function CreateGarage(shell_data)
        ...
        TeleportEntity(ped, shell_door_coords, shell_data.enter_heading, {skipGroundCheck = true})
    ...
    AddEventHandler('cd_garage:CancelCamOption', function()
        DrawTextUI('show', '<b>'..Locale('garage')..'<b/></p>'..Locale('cancel_cam'))
        while cam ~= nil do
            Wait(5)
