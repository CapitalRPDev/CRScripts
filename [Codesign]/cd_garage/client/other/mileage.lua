if Config.Mileage.ENABLE then
    ...
    if Config.Mileage.chat_command then
        TriggerEvent('chat:addSuggestion', '/'..Config.Mileage.chat_command, Locale('chatsuggestion_mileage'))
                ...
                if AdvStats.plate ~= nil then
                    if AdvStats.plate == plate then
                        ...
                        if Config.Mileage.speed_metrics == 'miles' then
                            Notif(2, 'miles_check_miles', RoundDecimals(AdvStats.mileage))
                        else
                            Notif(2, 'miles_check_kilometers', RoundDecimals(AdvStats.mileage))
                        end
    ...
    if Config.Mileage.ENABLE then
        if plate ~= nil then
            ...
            if AdvStatsTable[plate] ~= nil then
                ...
                AdvStatsTable[plate].mileage = RoundDecimals(AdvStatsTable[plate].mileage)
                return AdvStatsTable[plate]
            else
                local AdvStats = exports.cd_bridge:Callback('cd_garage:mileage', plate)
