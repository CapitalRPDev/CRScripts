...
AddEventHandler('cd_garage:VehicleManagement', function(action, data)
    ...
    if HasAdminPerms(_source, Config.StaffPerms[action].perms[Config.Framework]) then
...
function VehicleAdd(source, action, data)
    if GetPlayerName(data.target) then
        ...
        local Result = DB.fetch('SELECT plate FROM '..FW.vehicle_table..' WHERE plate="'..data.all_plates.trimmed..'" or plate="'..data.all_plates.with_spaces..'" or plate="'..data.all_plates.mixed..'"')
        if Result[1] == nil then
            if Config.Framework == 'esx' then
                DB.exec('INSERT INTO '..FW.vehicle_table..' ('..FW.vehicle_identifier..', plate, '..FW.vehicle_props..', garage_type, in_garage) VALUES (@'..FW.vehicle_identifier..', @plate, @'..FW.vehicle_props..', @garage_type, @in_garage)',
                {
            ...
            elseif Config.Framework == 'qbcore' or Config.Framework == 'qbox' then
                local vehicleData = GetSharedVehicles()
                DB.exec('INSERT INTO '..FW.vehicle_table..' ('..FW.vehicle_identifier..', plate, '..FW.vehicle_props..', garage_type, in_garage, license, hash, vehicle) VALUES (@'..FW.vehicle_identifier..', @plate, @'..FW.vehicle_props..', @garage_type, @in_garage, @license, @hash, @vehicle)',
                {
                    ['@'..FW.vehicle_identifier..''] = identifier,
                    ['@plate'] = data.plate,
                    ['@'..FW.vehicle_props..''] = json.encode(data.props),
                    ['@garage_type'] = data.garage_type,
                    ['@in_garage'] = 0,
                    ['@license'] = GetPlayerIdentifierByType(source, Config.IdentifierType),
                    ['@hash'] = data.props.model,
                    ['@vehicle'] = vehicleData[data.props.model] and vehicleData[data.props.model].model
                })
            end
...
function VehicleDelete(source, action, data)
    local Result = DB.fetch('SELECT plate FROM '..FW.vehicle_table..' WHERE plate="'..data.plate..'"')
    if Result and Result[1] then
        DB.exec('DELETE FROM '..FW.vehicle_table..' WHERE plate="'..data.plate..'"')
...
function VehiclePlate(source, action, data)
    local Result_1 = DB.fetch('SELECT plate, '..FW.vehicle_props..' FROM '..FW.vehicle_table..' WHERE plate="'..data.old_plate..'"')
    if Result_1 and Result_1[1] ~= nil then
        local Result_2 = DB.fetch('SELECT plate FROM '..FW.vehicle_table..' WHERE plate="'..data.new_plate..'"')
        if Result_2[1] == nil then
            local props = json.decode(Result_1[1][FW.vehicle_props])
            props.plate = data.new_plate
            DB.exec('UPDATE '..FW.vehicle_table..' SET plate=@new_plate, '..FW.vehicle_props..'=@'..FW.vehicle_props..' WHERE plate=@old_plate', {
