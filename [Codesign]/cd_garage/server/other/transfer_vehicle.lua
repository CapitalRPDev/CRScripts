if Config.TransferVehicle.ENABLE then
    ...
    AddEventHandler('cd_garage:TransferVehicle', function(plate, target, garage_type, label, fromUI)
        ...
        if GetPlayerName(target) ~= nil then
            if target ~= _source then
                ...
                if car_count < car_limit then
                    if CheckVehicleOwner(_source, plate) then
                        DB.exec('UPDATE '..FW.vehicle_table..' SET '..FW.vehicle_identifier..'="'..target_identifier..'" WHERE plate="'..plate..'"')
                        ...
                        if fromUI then
                            TriggerClientEvent('cd_garage:TransferVehicle:Callback', _source, true, Locale('transfer_success'), false)
                        end
                    ...
                    if fromUI then
                        TriggerClientEvent('cd_garage:TransferVehicle:Callback', _source, false, Locale('garage_full'), false)
                    else
                ...
                if fromUI then
                    TriggerClientEvent('cd_garage:TransferVehicle:Callback', _source, false, Locale('cant_transfer_yourself'), false)
                else
            ...
            if fromUI then
                TriggerClientEvent('cd_garage:TransferVehicle:Callback', _source, false, Locale('incorrect_id'), false)
            else
