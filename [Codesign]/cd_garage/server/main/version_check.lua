CreateThread(function()
    ...
    local download_link = 'https://portal.cfx.re/assets/granted-assets?search=cd_garage'
    PerformHttpRequest('https://raw.githubusercontent.com/RampBST/Codesign_Versions_V2/master/'..resource_name..'.txt',function(error, result, headers)
        if not result then Citizen.Trace('^1Version check disabled because github is having issues.^0\n') return end
        result = json.decode(result:sub(1, -2))
